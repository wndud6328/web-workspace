...
public class MemberDAO {
	 ...
	 * 					: int - DB에 데이터를 수정하고자 할 때 사용
	 * - delete(String mapper, Object param)
	 * 					: int - DB에 데이터를 삭제하고자 할 때 사용
	...
	public int registerMember(SqlSession sqlSession, MemberVO vo) {
		return sqlSession.insert("memberMapper.regiserMember", vo);
	}
	...
	public MemberVO findMyIdMember(SqlSession sqlSession, String id) {
		return sqlSession.selectOne("memberMapper.showAllmember");
	}
	
	public MemberVO login(SqlSession sqlSession, MemberVO vo){
		return sqlSession.selectOne("memberMapeeer.login", vo);
	}
